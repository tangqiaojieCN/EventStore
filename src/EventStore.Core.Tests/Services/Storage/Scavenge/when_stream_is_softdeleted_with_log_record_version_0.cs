...
namespace EventStore.Core.Tests.Services.Storage.Scavenge
{
    ...
    // TODO Hayley
    [TestFixture]
    public class when_stream_is_softdeleted_and_temp_with_log_version_0_but_some_events_are_in_multiple_chunks : ScavengeTestScenario
    {
        protected override DbResult CreateDb(TFChunkDbCreationHelper dbCreator)
        {
        ...
        {
            var headOfTf = new TFPos(Db.Config.WriterCheckpoint.Read(), Db.Config.WriterCheckpoint.Read());
            Assert.AreEqual(1, ReadIndex.ReadAllEventsForward(new TFPos(0, 0), 1000).Records.Count(x => x.Event.EventStreamId == "$$test"), "Read $$test stream forward");
            Assert.AreEqual(1, ReadIndex.ReadAllEventsBackward(headOfTf, 10).Records.Count(x => x.Event.EventStreamId == "$$test"), "Read $$test stream backward");
        }
    }
}
